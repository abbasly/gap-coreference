...
def calculate_scores(gold_annotations, system_annotations):
    ...
    for example_id, gold_annotation in gold_annotations.items():
        ...
        name_a_annotations = [
            gold_annotation.name_a_coref,
            system_annotation.name_a_coref,
        ]
        name_b_annotations = [
            gold_annotation.name_b_coref,
            system_annotation.name_b_coref,
        ]
...
def make_scorecard(scores):
    ...
    for gender, display_name in display_names:
        ...
        scorecard.append(
            "{} recall: {:.1f} precision: {:.1f} f1: {:.1f}".format(
...
def main(args):
    ...
    """
    # with open(args.system_tsv, 'wt') as out_file:
    #   tsv_writer = csv.writer(out_file, delimiter='\t')
    #   tsv_writer.writerow(['ID', 'A-coref', 'B-coref'])
    #   for i in range(2000):
    #     id_ = 'test-' + str(i+1)
    #     tsv_writer.writerow([id_, 'TRUE', 'FALSE'])
